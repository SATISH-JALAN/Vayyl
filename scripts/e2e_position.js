...
async function main() {
    ...
    const noteBlindness = Math.floor(Math.random() * 1000000).toString();
    ...
    const posBlindness = Math.floor(Math.random() * 1000000).toString();
    ...
    // 4. ATTEST HEALTH
    // ===================================================
    console.log("\n--- Attesting position health ---");

    // Update oracle price (price went up for our long)
    const healthTimestamp = Math.floor(Date.now() / 1000) + 60; // slightly in the future
    const setHealthCmd = `stellar contract invoke --id ${ORACLE_ID} --network ${NETWORK} --source ${SOURCE} -- set_price --price ${healthOraclePrice} --timestamp ${healthTimestamp}`;
    checkError(runCmd(setHealthCmd), false);
    console.log(`Oracle updated: price=${healthOraclePrice}, timestamp=${healthTimestamp}`);

    // Compute price_ge_entry selector: 1 if oracle_price >= entry_price
    const priceGeEntry = healthOraclePrice >= posEntryPrice ? "1" : "0";

    // health_threshold = 500 (5% maintenance margin, matches contract HEALTH_THRESHOLD const)
    const healthThreshold = "500";

    // Generate position_health proof
    const healthProofResult = generateProof("position_health", {
        position_commitment: posCommitment,
        oracle_price: String(healthOraclePrice),
        oracle_timestamp: String(healthTimestamp),
        ...
        pubY: pubY,
        position_blindness: posBlindness,
        price_ge_entry: priceGeEntry
    });

    // Call attest_health on position manager
    ...
    console.log(`\x1b[32m✅ Health attested! Oracle price: ${healthOraclePrice}\x1b[0m`);
    ...
    // Get pool leaf count before close
    const leafCountBefore = Number(
        parseInvokeOutput(
            runCmd(
                `stellar contract invoke --id ${POOL_ID} --network ${NETWORK} --source ${SOURCE} -- get_leaf_count`
            ).output
    ...
    console.log(`Pool leaf count before close: ${leafCountBefore}`);

    // PnL calculation (long, entry 2000, close 2500, size 500):
    //   For direction=1 (Long):
    //     asset_val = direction * (oracle_price - entry_price) + entry_price
    //               = 1 * (2500 - 2000) + 2000 = 2500
    //     debt_val  = direction * (entry_price - oracle_price) + oracle_price
    //               = 1 * (2000 - 2500) + 2500 = 2000
    //     lhs = old_collateral + old_size * asset_val = 1000 + 500 * 2500 = 1251000
    //     rhs = new_collateral + note_amount + fee + old_size * debt_val
    //         = 0 + note_amount + 0 + 500 * 2000 = note_amount + 1000000
    //     lhs === rhs → note_amount = 1251000 - 1000000 = 251000
    const closeFee = "0";
    const noteAmount = "251000";
    const closeMetaHash = getMetaHash(deployerAddr, closeFee);

    // New position: fully closed (size=0, collateral=0)
    const newCollateral = "0";
    const newSize = "0";
    const newDirection = "0";
    const newEntryPrice = "0";
    const newBlindness = "55555";

    // Output note params
    const notePubX = pubX;
    const notePubY = pubY;
    const noteOutBlindness = "33333";

    // Compute commitments
    const newPosCommitment = calculatePositionCommitment(
        newCollateral, pubX, pubY, newSize, newDirection, newEntryPrice, newBlindness
    );
    const outputNoteCommitment = calculateHash4([noteAmount, notePubX, notePubY, noteOutBlindness]);

    console.log(`New position commitment: ${newPosCommitment}`);
    console.log(`Output note commitment:  ${outputNoteCommitment}`);
    console.log(`Note amount (PnL):       ${noteAmount}`);

    // Generate position_close proof
    const closeProofResult = generateProof("position_close", {
        position_nullifier: posNullifier,
        new_position_commitment: newPosCommitment,
        output_note_commitment: outputNoteCommitment,
        oracle_price: String(closeOraclePrice),
        fee: closeFee,
        meta_hash: closeMetaHash,
        old_collateral: collateralAmount,
        old_size: posSize,
        old_direction: posDirection,
        old_entry_price: posEntryPrice,
        old_pubX: pubX,
        old_pubY: pubY,
        old_blindness: posBlindness,
        old_privKey: priv,
        new_collateral: newCollateral,
        new_size: newSize,
        new_direction: newDirection,
        new_entry_price: newEntryPrice,
        new_pubX: pubX,
        new_pubY: pubY,
        new_blindness: newBlindness,
        note_amount: noteAmount,
        note_pubX: notePubX,
        note_pubY: notePubY,
        note_blindness: noteOutBlindness
    });

    // Call close_or_modify_position on position manager
    const closeCmd = `stellar contract invoke --id ${POSITION_MANAGER_ID} --network ${NETWORK} --source ${SOURCE}` +
        ` -- close_or_modify_position` +
        ` --position_id "{\\\"bytes\\\":\\\"${positionIdHex}\\\"}"` +
        ` --proof "${closeProofResult.proofStr.replace(/"/g, '\\"')}"` +
        ` --position_nullifier "{\\\"bytes\\\":\\\"${formatFr(closeProofResult.publicInputs[0])}\\\"}"` +
        ` --new_position_commitment "{\\\"bytes\\\":\\\"${formatFr(closeProofResult.publicInputs[1])}\\\"}"` +
        ` --output_note_commitment "{\\\"bytes\\\":\\\"${formatFr(closeProofResult.publicInputs[2])}\\\"}"` +
        ` --fee ${closeFee}` +
        ` --meta_hash "{\\\"bytes\\\":\\\"${formatFr(closeProofResult.publicInputs[5])}\\\"}"`
    ;

    checkError(runCmd(closeCmd), false);
    console.log(`\x1b[32m✅ Position closed! PnL settled: ${noteAmount} (output note in pool)\x1b[0m`);

    // Verify pool leaf count increased (output note inserted via execute_settlement)
    const leafCountAfter = Number(
        parseInvokeOutput(
            runCmd(
                `stellar contract invoke --id ${POOL_ID} --network ${NETWORK} --source ${SOURCE} -- get_leaf_count`
            ).output
    ...
    console.log(`Pool leaf count after close: ${leafCountAfter}`);
    if (leafCountAfter <= leafCountBefore) {
        console.error(`\x1b[31m❌ Pool leaf count did not increase after close! before=${leafCountBefore} after=${leafCountAfter}\x1b[0m`);
        process.exit(1);
    }
    console.log(`\x1b[32m✅ Pool leaf count increased by ${leafCountAfter - leafCountBefore}\x1b[0m`);

    // ===================================================
    // SUMMARY
    // ===================================================
    console.log("\n\x1b[32m=== Sprint 1 — Position Vertical E2E PASSED! ===\x1b[0m");
    console.log(`Position lifecycle: deposit → open → attest_health → close`);
    console.log(`Oracle prices: entry=${entryOraclePrice}, health=${healthOraclePrice}, close=${closeOraclePrice}`);
    console.log(`PnL settled: ${noteAmount} (output note in pool tree)`);
    console.log(`Pool leaf count: ${leafCountBefore} → ${leafCountAfter}`);
}
