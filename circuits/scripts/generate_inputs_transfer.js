...
function calculateHash4(inputs) {
    ...
    runCmd(`snarkjs wtns calculate build/hash4_js/hash4.wasm test/hash4_input.json build/hash4.wtns`);
...
function calculateHash2(inputs) {
    ...
    fs.writeFileSync(inputPath, JSON.stringify({ in: inputs }));
    runCmd(`snarkjs wtns calculate build/hash2_js/hash2.wasm test/hash2_input.json build/hash2.wtns`);
...
function main() {
    ...
    // Empty-subtree ladder
    const zeros = ["0"];
    for (let l = 1; l <= depth; l++) {
        zeros[l] = calculateHash2([zeros[l-1], zeros[l-1]]);
    }
    ...
    for (let i = 0; i < depth; i++) {
        asp_pathElements.push(zeros[i]);
        asp_pathIndices.push(0);
        current = calculateHash2([current, zeros[i]]);
    }
